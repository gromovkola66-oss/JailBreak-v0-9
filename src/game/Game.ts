...
import { GarageDoorSystem } from './GarageDoorSystem';
...
export class Game {
  ...
  private onGarageDoorLockUpdate?: (state: { state: 'unlocked' | 'locked' | 'temp_locked'; remainingSeconds: number | null }) => void;
  ...
  setOnGarageDoorLockUpdate(callback: (state: { state: 'unlocked' | 'locked' | 'temp_locked'; remainingSeconds: number | null }) => void) {
    this.onGarageDoorLockUpdate = callback;
    callback(this.garageDoorSystem.getLockState());
  }
  ...
  private animate() {
    ...
    // Update garage door lock system
