...
export class Hands {
  ...
  // Wall proximity retraction
  private wallProximity: number = 999;
  ...
  setWallProximity(distance: number) { this.wallProximity = distance; }
  ...
  update(delta: number) {
    ...
    // Wall proximity retraction
    if (this.wallProximity < 0.55) {
      const retractAmount = Math.min((0.55 - this.wallProximity) * 1.5, 0.4);
      this.leftArm.position.z += retractAmount;
      this.rightArm.position.z += retractAmount;
      this.leftArm.position.y += retractAmount * 0.3;
      this.rightArm.position.y += retractAmount * 0.3;
      this.leftArm.rotation.x += retractAmount * 0.5;
      this.rightArm.rotation.x += retractAmount * 0.5;
      if (this.itemModel) {
        this.itemModel.position.z += retractAmount;
        this.itemModel.position.y += retractAmount * 0.3;
      }
    }
  }
}
