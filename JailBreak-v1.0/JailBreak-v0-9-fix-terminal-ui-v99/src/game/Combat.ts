...
export class Combat {
  ...
  // Bullet hole pooling - limit to MAX_BULLET_HOLES to avoid runaway mesh count
  private static readonly MAX_BULLET_HOLES = 30;
  private bulletHoles: THREE.Mesh[] = [];
  private bulletHoleGeometry = new THREE.CircleGeometry(0.05, 6);
  private bulletHoleMaterial = new THREE.MeshBasicMaterial({ 
    color: 0x111111,
  ...
  private createBulletHole(position: THREE.Vector3, normal?: THREE.Vector3) {
    ...
    if (this.bulletHoles.length >= Combat.MAX_BULLET_HOLES) {
      // Reuse oldest bullet hole
      hole = this.bulletHoles.shift()!;
    } else {
      hole = new THREE.Mesh(this.bulletHoleGeometry, this.bulletHoleMaterial);
      this.scene.add(hole);
    }
  ...
  // Pre-allocated box for dropped object collision checks
  private readonly _dropBox = new THREE.Box3(new THREE.Vector3(), new THREE.Vector3());
  ...
  private updateDroppedObjectPhysics(obj: THREE.Group, delta: number) {
    ...
    // Use a small fixed bounding box around the object center instead of expensive setFromObject
    const hw = 0.2; // half-width approximation
    const hh = 0.15; // half-height approximation

    // Check Y collision against map objects
    if (obj.userData.velocityY < 0) {
      const deltaY = obj.userData.velocityY * delta;
      this._dropBox.min.set(obj.position.x - hw, obj.position.y - hh + deltaY, obj.position.z - hw);
      this._dropBox.max.set(obj.position.x + hw, obj.position.y + hh + deltaY, obj.position.z + hw);
      let landedOnObject = false;
      for (const collider of this.mapColliders) {
        if (this._dropBox.intersectsBox(collider) && collider.max.y <= obj.position.y) {
    ...
    if (!obj.userData.grounded) {
      // Check X collision
      this._dropBox.min.set(obj.position.x - hw + obj.userData.velocityX * delta, obj.position.y - hh, obj.position.z - hw);
      this._dropBox.max.set(obj.position.x + hw + obj.userData.velocityX * delta, obj.position.y + hh, obj.position.z + hw);
      let hitX = false;
      for (const collider of this.mapColliders) {
        if (this._dropBox.intersectsBox(collider)) { hitX = true; break; }
      }
      if (!hitX) obj.position.x = newX;
      else obj.userData.velocityX = 0;

      // Check Z collision
      this._dropBox.min.set(obj.position.x - hw, obj.position.y - hh, obj.position.z - hw + obj.userData.velocityZ * delta);
      this._dropBox.max.set(obj.position.x + hw, obj.position.y + hh, obj.position.z + hw + obj.userData.velocityZ * delta);
      let hitZ = false;
      for (const collider of this.mapColliders) {
        if (this._dropBox.intersectsBox(collider)) { hitZ = true; break; }
      }
