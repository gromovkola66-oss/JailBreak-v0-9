...
export class CameraSystem {
  ...
  // Cycle through terminals one-at-a-time to amortize render cost
  private terminalRenderIndex = 0;
  ...
  constructor(scene: THREE.Scene, renderer: THREE.WebGLRenderer) {
    ...
    this.terminalScreenTarget = new THREE.WebGLRenderTarget(128, 72);
  ...
  private applyHighlight(group: THREE.Object3D) {
    group.traverse((child) => {
      if (child instanceof THREE.Mesh) {
        ...
        cloned.emissiveIntensity = 0.008;
  ...
  updateTerminalScreen(delta: number) {
    ...
    // Reduced update frequency (1.0s instead of 0.5s) for non-critical terminal preview
    if (this.terminalScreenElapsed < 1.0) return;
    ...
    // Only render ONE terminal per tick to amortize cost across frames
    const terminalIdx = this.terminalRenderIndex % this.terminals.length;
    this.terminalRenderIndex++;

    const terminal = this.terminals[terminalIdx];
    ...
    this.securityCamera.position.copy(cam.position);
    this.securityCamera.rotation.copy(cam.rotation);
    this.securityCamera.aspect = 128 / 72;
    this.securityCamera.updateProjectionMatrix();
    ...
    // Copy pixels into a per-terminal DataTexture
