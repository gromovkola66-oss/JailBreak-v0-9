...
export class PlaytestMode {
  ...
  constructor(container: HTMLElement, mapData: MapData, team: 'guard' | 'prisoner') {
    ...
    if (isFogEnabled()) {
      this.scene.fog = new THREE.Fog(0xc8e0f0, 15, 120);
    }
    ...
    // Камера — reduced far plane for better depth precision and GPU culling
    ...
    configureSunShadow(this.sunLight, 30);
  ...
  private loadMap(mapData: MapData, team: 'guard' | 'prisoner') {
    ...
    // Limit the number of dynamic lights to avoid GPU overload
    const MAX_DYNAMIC_LIGHTS = 8;
    let lightCount = 0;
    for (const obj of objectsToRemove) {
      obj.traverse((node) => {
        if (node instanceof THREE.Light && lightCount < MAX_DYNAMIC_LIGHTS) {
          const light = node.clone();
          const worldPos = new THREE.Vector3();
          node.getWorldPosition(worldPos);
          light.position.copy(worldPos);
          // Disable shadows for all extracted lights (huge performance save)
          light.castShadow = false;
          if (light instanceof THREE.PointLight) {
            light.intensity *= 2;
            light.distance = Math.min(light.distance || 10, 10);
            light.decay = 2;
          } else if (light instanceof THREE.SpotLight) {
            light.intensity *= 2;
            light.distance = Math.min(light.distance || 10, 12);
          }
