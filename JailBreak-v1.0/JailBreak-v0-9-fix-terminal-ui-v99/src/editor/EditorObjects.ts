...
function cyl(rt: number, rb: number, h: number, mat: THREE.Material, seg = 6): THREE.Mesh {
...
export const EDITOR_OBJECTS: EditorObjectType[] = [
  // ============ СТЕНЫ / ПОЛ / ПОТОЛОК ============
  {
    ...
    create: () => {
      ...
      // Reduced detail: only 2 seam lines instead of 4+
      g.add(pos(box(0.02, 4, 0.005, M.concDirty), -1, 2, 0.148));
      g.add(pos(box(0.02, 4, 0.005, M.concDirty), 1, 2, 0.148));
  ...
  },
  {
    ...
    create: () => {
      ...
      // Simplified: just 2 cross seams instead of full grid
      g.add(pos(box(0.02, 0.01, 4, M.concDirty), 0, 0.01, 0));
      g.add(pos(box(4, 0.01, 0.02, M.concDirty), 0, 0.01, 0));
      return g;
    }
  },
  {
  ...
  // --- Кирпичная стена ---
  {
    ...
    create: () => {
      ...
      // Simplified mortar: just horizontal lines every 0.6
      for (let y = 0.3; y < 4; y += 0.6) {
        g.add(pos(box(4, 0.02, 0.005, M.mortar), 0, y, 0.148));
      }
      // A few color variation patches instead of per-brick
      g.add(pos(box(1.5, 1.2, 0.005, M.brick2), -0.8, 1.5, 0.148));
      g.add(pos(box(1.2, 1.0, 0.005, M.brick3), 1.0, 3.0, 0.148));
      g.add(pos(box(4, 0.15, 0.3, M.conc3), 0, 0.075, 0));
      return g;
    }
  },
  // --- Стена из бетонных блоков ---
  {
    ...
    create: () => {
      ...
      // Simplified: just horizontal seams every 0.8
      for (let y = 0; y < 4; y += 0.8) {
        g.add(pos(box(4, 0.02, 0.005, M.cblockEdge), 0, y, 0.148));
      }
      // 2 vertical seams
      g.add(pos(box(0.02, 4, 0.005, M.cblockEdge), -0.8, 2, 0.148));
      g.add(pos(box(0.02, 4, 0.005, M.cblockEdge), 0.8, 2, 0.148));
  ...
  // --- Стена с плиткой ---
  {
    ...
    create: () => {
      ...
      // Simplified grout - just 3 horizontal + 3 vertical
      for (let y = 1; y < 4; y += 1) {
  ...
  // --- Плиточный пол ---
  {
    ...
    create: () => {
      const g = new THREE.Group();
      g.add(pos(box(4, 0.3, 4, M.conc2), 0, -0.15, 0));
      // Simplified checkerboard: 4 tiles instead of 16+
      g.add(pos(box(1.9, 0.02, 1.9, M.tileWhite), -0.95, 0.01, -0.95));
      g.add(pos(box(1.9, 0.02, 1.9, M.tileDark), 0.95, 0.01, -0.95));
      g.add(pos(box(1.9, 0.02, 1.9, M.tileDark), -0.95, 0.01, 0.95));
      g.add(pos(box(1.9, 0.02, 1.9, M.tileWhite), 0.95, 0.01, 0.95));
      // 2 cross grouting lines
      g.add(pos(box(0.03, 0.01, 4, M.tileGrout), 0, 0.015, 0));
      g.add(pos(box(4, 0.01, 0.03, M.tileGrout), 0, 0.015, 0));
      return g;
    }
  },
  // --- Потолок панельный ---
  {
    ...
    create: () => {
      ...
      // Simplified: 4 panels instead of 16
      g.add(pos(box(1.8, 0.04, 1.8, M.ceilPanel), -0.95, 3.98, -0.95));
      g.add(pos(box(1.8, 0.04, 1.8, M.ceilPanel), 0.95, 3.98, -0.95));
      g.add(pos(box(1.8, 0.04, 1.8, M.ceilPanel), -0.95, 3.98, 0.95));
      g.add(pos(box(1.8, 0.04, 1.8, M.ceilPanel), 0.95, 3.98, 0.95));
      // Cross frame
      g.add(pos(box(0.06, 0.06, 4, M.ceilGrid), 0, 3.97, 0));
      g.add(pos(box(4, 0.06, 0.06, M.ceilGrid), 0, 3.97, 0));
      return g;
    }
  },
  // --- Потолок ржавый ---
  ...
  // --- Сетка-рабица ---
  {
    ...
    create: () => {
      ...
      // Simplified fence: only 6 diagonal wires instead of 24
      for (let i = 0; i < 6; i++) {
        const w = box(0.015, 0.015, 2.7, M.meshWire);
        w.position.set(-0.75 + i * 0.3, 1.5, 0);
        w.rotation.z = 0.7;
        g.add(w);
        const w2 = box(0.015, 0.015, 2.7, M.meshWire);
        w2.position.set(-0.75 + i * 0.3, 1.5, 0);
        w2.rotation.z = -0.7;
        g.add(w2);
      }
      return g;
