...
const GameApp = ({ onBackToMenu }: GameAppProps) => {
  ...
  const [garageDoorLockState, setGarageDoorLockState] = useState<{ state: 'unlocked' | 'locked' | 'temp_locked'; remainingSeconds: number | null }>({ state: 'unlocked', remainingSeconds: null });
  ...
  useEffect(() => {
    ...
    game.setOnGarageDoorLockUpdate((state) => {
      setGarageDoorLockState({ ...state });
    });
    ...
    <div className="w-screen h-screen overflow-hidden bg-black">
      ...
      {teamSelected && (
        <GameUI 
          ...
          onOpenTerminalApp={(app) => gameRef.current?.openTerminalApp(app)}
          onBackToTerminalDesktop={() => gameRef.current?.backToTerminalDesktop()}
          garageDoorLockState={garageDoorLockState}
          onLockGarageDoors={() => gameRef.current?.lockGarageDoors()}
          onUnlockGarageDoors={() => gameRef.current?.unlockGarageDoors()}
          onTempLockGarageDoors={(minutes) => gameRef.current?.tempLockGarageDoors(minutes)}
