...
export class FirstPersonController {
  ...
  // Pre-allocated vectors reused every frame to avoid GC
  private readonly _forward = new THREE.Vector3();
  private readonly _right = new THREE.Vector3();
  private readonly _newPosX = new THREE.Vector3();
  private readonly _newPosZ = new THREE.Vector3();
  private readonly _steppedPos = new THREE.Vector3();
  ...
  private readonly _playerBox = new THREE.Box3(new THREE.Vector3(), new THREE.Vector3());

  private checkCollision(newPosition: THREE.Vector3): boolean {
    const h = this.currentHeight;
    const feetClearance = 0.05;
    const feetY = newPosition.y - h;
    // Reuse pre-allocated Box3
    this._playerBox.min.set(newPosition.x - 0.3, feetY + feetClearance, newPosition.z - 0.3);
    this._playerBox.max.set(newPosition.x + 0.3, newPosition.y + 0.2, newPosition.z + 0.3);
    for (let i = 0, len = this.colliders.length; i < len; i++) {
  ...
  update(delta: number) {
    ...
    if (this.isLocked) {
      ...
      moveX = (this._forward.x * this.direction.z + this._right.x * this.direction.x) * speed * delta;
      moveZ = (this._forward.z * this.direction.z + this._right.z * this.direction.x) * speed * delta;
    }

    // Horizontal collision with step-up
    const stepUpHeight = 0.55;

    this._newPosX.copy(this.camera.position); this._newPosX.x += moveX;
    if (this.checkCollision(this._newPosX)) {
      // Try stepping up
      this._steppedPos.copy(this._newPosX);
      this._steppedPos.y += stepUpHeight;
      if (!this.checkCollision(this._steppedPos)) {
        this.camera.position.x = this._newPosX.x;
    ...
    this._newPosZ.copy(this.camera.position); this._newPosZ.z += moveZ;
    if (this.checkCollision(this._newPosZ)) {
      // Try stepping up
      this._steppedPos.copy(this._newPosZ);
      this._steppedPos.y += stepUpHeight;
      if (!this.checkCollision(this._steppedPos)) {
        this.camera.position.z = this._newPosZ.z;
        const feetYForStep = this.camera.position.y - this.currentHeight;
        for (let i = 0, len = this.colliders.length; i < len; i++) {
          const collider = this.colliders[i];
          if (this.camera.position.x + 0.3 > collider.min.x &&
