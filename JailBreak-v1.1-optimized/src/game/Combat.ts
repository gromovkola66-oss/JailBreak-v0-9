...
export class Combat {
  ...
  private shoot() {
    ...
    if (this.weapon.fire()) {
      ...
      this._raycaster.set(this.camera.position, direction);
      this._raycaster.far = this.weapon.stats.range;
  ...
  private bulletHoleIndex = 0;
  private static readonly MAX_BULLET_HOLES = 30;
  private static readonly bulletHoleGeometry = new THREE.CircleGeometry(0.05, 6);
  private static readonly bulletHoleMaterial = new THREE.MeshBasicMaterial({ 
    color: 0x111111,
  ...
  private createBulletHole(position: THREE.Vector3, normal?: THREE.Vector3) {
    ...
    if (this.bulletHoles.length < Combat.MAX_BULLET_HOLES) {
      // Create new mesh from shared geometry/material
      hole = new THREE.Mesh(Combat.bulletHoleGeometry, Combat.bulletHoleMaterial);
      this.bulletHoles.push(hole);
      this.scene.add(hole);
    } else {
      // Reuse oldest bullet hole (ring buffer)
      hole = this.bulletHoles[this.bulletHoleIndex];
    }
    ...
    this.bulletHoleIndex = (this.bulletHoleIndex + 1) % Combat.MAX_BULLET_HOLES;
  }
  ...
  // Pre-allocated objects for physics to avoid per-frame allocations
  private static readonly _physTestBox = new THREE.Box3();
  ...
  private updateDroppedObjectPhysics(obj: THREE.Group, delta: number) {
    ...
    // Use a simple AABB around object center instead of expensive setFromObject
    const halfSize = 0.15;
    const testBox = Combat._physTestBox;

    // Check Y collision against map objects
    if (obj.userData.velocityY < 0) {
      testBox.min.set(obj.position.x - halfSize, newY - halfSize, obj.position.z - halfSize);
      testBox.max.set(obj.position.x + halfSize, newY + halfSize, obj.position.z + halfSize);
      let landedOnObject = false;
      for (let i = 0, len = this.mapColliders.length; i < len; i++) {
        const collider = this.mapColliders[i];
        if (testBox.intersectsBox(collider) && collider.max.y <= obj.position.y) {
    ...
    if (!obj.userData.grounded) {
      // Check X collision
      testBox.min.set(newX - halfSize, obj.position.y - halfSize, obj.position.z - halfSize);
      testBox.max.set(newX + halfSize, obj.position.y + halfSize, obj.position.z + halfSize);
      let hitX = false;
      for (let i = 0, len = this.mapColliders.length; i < len; i++) {
        if (testBox.intersectsBox(this.mapColliders[i])) { hitX = true; break; }
      }
      if (!hitX) obj.position.x = newX;
      else obj.userData.velocityX = 0;

      // Check Z collision
      testBox.min.set(obj.position.x - halfSize, obj.position.y - halfSize, newZ - halfSize);
      testBox.max.set(obj.position.x + halfSize, obj.position.y + halfSize, newZ + halfSize);
      let hitZ = false;
      for (let i = 0, len = this.mapColliders.length; i < len; i++) {
        if (testBox.intersectsBox(this.mapColliders[i])) { hitZ = true; break; }
      }
