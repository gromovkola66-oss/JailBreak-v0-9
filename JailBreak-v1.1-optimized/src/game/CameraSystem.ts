...
export class CameraSystem {
  ...
  registerTerminal(id: string, mesh: THREE.Object3D, position: THREE.Vector3, groupId: number) {
    ...
    this._terminalMeshesDirty = true;
  }
  ...
  // Cached terminal meshes to avoid per-frame allocation
  private _terminalMeshes: THREE.Object3D[] = [];
  private _terminalMeshesDirty = true;

  private rebuildTerminalMeshCache() {
    this._terminalMeshes.length = 0;
    for (const terminal of this.terminals) {
      terminal.mesh.traverse((child) => {
        if (child instanceof THREE.Mesh && !child.userData.isTerminalScreen) {
          this._terminalMeshes.push(child);
        }
      });
    }
    this._terminalMeshesDirty = false;
  }

  checkRaycast(raycaster: THREE.Raycaster): boolean {
    if (this._terminalMeshesDirty) {
      this.rebuildTerminalMeshCache();
    }

    if (this._terminalMeshes.length === 0) {
      this.clearHighlight();
    ...
    const intersects = raycaster.intersectObjects(this._terminalMeshes, false);
