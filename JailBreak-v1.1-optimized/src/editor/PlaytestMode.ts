...
export class PlaytestMode {
  ...
  constructor(container: HTMLElement, mapData: MapData, team: 'guard' | 'prisoner') {
    ...
    // Sky dome — reduced segments for performance (16x16 instead of 32x32)
    ...
    const skyMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        // Simplified hash for stars/clouds - cheaper than full fbm
        float hash(vec3 p) {
        ...
        // Reduced to 2 octaves from 4 for performance
        float fbm(vec3 p) {
          float v = 0.0;
          v += 0.5 * noise(p);
          v += 0.25 * noise(p * 2.0);
        ...
        void main() {
          ...
          sky += vec3(1.0, 0.95, 0.8) * (sunDisc + sunGlow) * step(0.0, sunElev);
          ...
          sky += vec3(0.8, 0.85, 1.0) * moonDisc * nightFactor;

          // Stars (simplified - no twinkle math)
          if (h > 0.05 && nightFactor > 0.1) {
            vec3 starCoord = dir * 200.0;
            float starHash = hash(floor(starCoord));
            float starBright = step(0.988, starHash);
            sky += vec3(starBright * nightFactor * 0.8);
          }

          // Clouds (reduced octaves, skip if below horizon)
          if (h > 0.05) {
            vec3 cloudCoord = dir / max(h, 0.05) * 3.0;
            float cloud = fbm(cloudCoord + vec3(uElapsedSeconds * 0.02, 0.0, uElapsedSeconds * 0.005));
            cloud = smoothstep(0.42, 0.7, cloud);
            float cloudAlpha = cloud * 0.5 * smoothstep(0.05, 0.15, h);
  ...
  private loadMap(mapData: MapData, team: 'guard' | 'prisoner') {
    ...
    // PERFORMANCE: Limit total extracted point lights to avoid GPU overload
    const MAX_EXTRACTED_LIGHTS = 16;
    const extractedLights: THREE.Light[] = [];
    for (const obj of objectsToRemove) {
      obj.traverse((node) => {
        if (node instanceof THREE.Light && extractedLights.length < MAX_EXTRACTED_LIGHTS) {
          const light = node.clone();
          const worldPos = new THREE.Vector3();
          node.getWorldPosition(worldPos);
          light.position.copy(worldPos);
          light.castShadow = false;
          // Keep intensity reasonable - too many bright lights kills FPS
          if (light instanceof THREE.PointLight) {
            light.intensity = Math.min(light.intensity * 2, 3);
            light.distance = Math.max(light.distance, 12);
            light.decay = 2; // Physical falloff for better performance
          } else if (light instanceof THREE.SpotLight) {
            light.intensity = Math.min(light.intensity * 1.5, 3);
          }
