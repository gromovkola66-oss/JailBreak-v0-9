...
export const EDITOR_OBJECTS: EditorObjectType[] = [
  ...
  // --- Кирпичная стена ---
  {
    ...
    create: () => {
      ...
      // Only a few accent patches instead of per-brick mortar lines
      g.add(pos(box(0.75, 0.26, 0.005, M.brick2), -0.8, 1.5, 0.148));
      g.add(pos(box(0.75, 0.26, 0.005, M.brick3), 0.6, 2.8, 0.148));
      g.add(pos(box(4, 0.15, 0.3, M.conc3), 0, 0.075, 0));
      return g;
    }
  },
  ...
  // --- Стена с плиткой ---
  {
    ...
    create: () => {
      ...
      // Just colored bands instead of per-tile grout lines
  ...
  // --- Деревянный пол ---
  {
    ...
    create: () => {
      ...
      // Just 2 material bands to indicate wood planks
      g.add(pos(box(4, 0.04, 2, M.woodFloor1), 0, 0.0, -1));
      g.add(pos(box(4, 0.04, 2, M.woodFloor2), 0, 0.0, 1));
      return g;
    }
  },
  // --- Плиточный пол ---
  {
    ...
    create: () => {
      const g = new THREE.Group();
      g.add(pos(box(4, 0.3, 4, M.conc2), 0, -0.15, 0));
      // Simplified checkerboard: 4 tiles instead of 16+grouting
      g.add(pos(box(1.95, 0.02, 1.95, M.tileWhite), -1, 0.01, -1));
      g.add(pos(box(1.95, 0.02, 1.95, M.tileDark), 1, 0.01, -1));
      g.add(pos(box(1.95, 0.02, 1.95, M.tileDark), -1, 0.01, 1));
      g.add(pos(box(1.95, 0.02, 1.95, M.tileWhite), 1, 0.01, 1));
      return g;
    }
  ...
  // --- Потолок панельный ---
  {
    ...
    create: () => {
      ...
      g.add(pos(box(4, 0.04, 4, M.ceilPanel), 0, 3.98, 0));
      return g;
    }
  },
  // --- Потолок ржавый ---
  ...
  // --- Толстая решётка ---
  {
    ...
    create: () => {
      ...
      // Simplified: 3 horizontal bars instead of 5+welds
  ...
  // --- Ржавая решётка ---
  {
    ...
    create: () => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        g.add(pos(cyl(0.02, 0.02, 3, m), -0.55 + i * 0.18, 1.5, 0));
      }
      for (const y of [0.3, 1.5, 2.7]) {
  ...
  // --- Сетка-рабица ---
  {
    ...
    create: () => {
      ...
      // Simplified mesh — single semi-transparent panel
      const meshPanel = new THREE.Mesh(
        geoCache.get('b_1.9_2.85_0.02') ?? (() => { const geo = new THREE.BoxGeometry(1.9, 2.85, 0.02); geoCache.set('b_1.9_2.85_0.02', geo); return geo; })(),
        M.chainlink
      );
      meshPanel.position.set(0, 1.5, 0);
      meshPanel.castShadow = false;
      g.add(meshPanel);
      return g;
    }
  },
  ...
  // --- Industrial Metal Wall Panel ---
  {
    ...
    create: () => {
      ...
      // Simplified seams (just 2 main lines)
      g.add(pos(box(4, 0.03, 0.01, M.metalDark), 0, 2.0, 0.126));
      g.add(pos(box(0.03, 4, 0.01, M.metalDark), 0, 2, 0.126));
      // Rust patches (2 instead of 5)
      g.add(pos(box(0.5, 0.4, 0.005, M.rustPatch), -1.0, 0.8, 0.128));
      g.add(pos(box(0.4, 0.3, 0.005, M.rustPatch), 1.5, 2.5, 0.128));
      // Green paint lower section
  ...
  // --- Ventilation Duct Section ---
  {
    ...
    create: () => {
      ...
      // Mounting brackets (simplified - 2 instead of 3 full bracket assemblies)
  ...
  // ============ ПРЕДМЕТЫ ============
  {
    ...
    create: () => {
      ...
      g.add(pos(box(0.04, 2, 0.04, M.metalDark), 0.95, 1, -0.3));
      for (let i = 0; i < 4; i++) {
        g.add(pos(box(0.04, 0.04, 0.56, M.metalLight), 0.95, 0.3 + i * 0.5, 0));
      }
