        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {activeCategory === 'terrain' && (
            <div className="p-2 border-b border-gray-700 mb-2">
              <div className="mb-3">
                <div className="text-xs text-gray-400 mb-1">Размер карты</div>
                <div className="grid grid-cols-5 gap-1">
                  {[200, 400, 600, 800, 1000].map(size => (
                    <button key={size} onClick={() => onTerrainResize(size)}
                      className={`py-1.5 rounded text-xs text-center transition font-mono ${terrainSize === size ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
                      {size}
                    </button>
                  ))}
                </div>
