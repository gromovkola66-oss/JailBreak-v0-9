...
export const EDITOR_OBJECTS: EditorObjectType[] = [
  ...
  },
  {
    ...
    create: () => {
      ...
      // Invisible collision meshes for pillars and crossbar only
      const collLeft = new THREE.Mesh(
        new THREE.BoxGeometry(0.12, 3.5, 0.3),
        new THREE.MeshBasicMaterial({ visible: false })
      );
      collLeft.position.set(-0.6, 1.75, 0);
      g.add(collLeft);
      const collRight = new THREE.Mesh(
        new THREE.BoxGeometry(0.12, 3.5, 0.3),
        new THREE.MeshBasicMaterial({ visible: false })
      );
      collRight.position.set(0.6, 1.75, 0);
      g.add(collRight);
      const collTop = new THREE.Mesh(
        new THREE.BoxGeometry(1.32, 0.12, 0.3),
        new THREE.MeshBasicMaterial({ visible: false })
      );
      collTop.position.set(0, 3.55, 0);
      g.add(collTop);
      g.userData.hasCustomColliders = true;
      return mergeGroup(g);
    }
  },
  {
  ...
  // --- Стена с дверным проёмом ---
  {
    ...
    create: () => {
      ...
      g.add(pos(box(0.08, 3.5, 0.32, M.metalDark), 0.68, 1.75, 0));
      // Invisible collision meshes for solid parts only
      const collLeft = new THREE.Mesh(
        new THREE.BoxGeometry(1.2, 4, 0.3),
        new THREE.MeshBasicMaterial({ visible: false })
      );
      collLeft.position.set(-1.4, 2, 0);
      g.add(collLeft);
      const collRight = new THREE.Mesh(
        new THREE.BoxGeometry(1.2, 4, 0.3),
        new THREE.MeshBasicMaterial({ visible: false })
      );
      collRight.position.set(1.4, 2, 0);
      g.add(collRight);
      const collTop = new THREE.Mesh(
        new THREE.BoxGeometry(1.5, 0.5, 0.3),
        new THREE.MeshBasicMaterial({ visible: false })
      );
      collTop.position.set(0, 3.75, 0);
      g.add(collTop);
      const collFrameLeft = new THREE.Mesh(
        new THREE.BoxGeometry(0.08, 3.5, 0.32),
        new THREE.MeshBasicMaterial({ visible: false })
      );
      collFrameLeft.position.set(-0.68, 1.75, 0);
      g.add(collFrameLeft);
      const collFrameRight = new THREE.Mesh(
        new THREE.BoxGeometry(0.08, 3.5, 0.32),
        new THREE.MeshBasicMaterial({ visible: false })
      );
      collFrameRight.position.set(0.68, 1.75, 0);
      g.add(collFrameRight);
      g.userData.hasCustomColliders = true;
      return mergeGroup(g);
    }
  },
