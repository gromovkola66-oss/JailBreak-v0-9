...
import { LightingSystem, LightProperties } from './LightingSystem';
import { VolumetricLightSystem } from './VolumetricLight';
import { AnimationSystem, AnimationData } from './AnimationSystem';
import { ScriptGraph } from './NodeEditor';

import { ProceduralGenerator, GenerationParams } from './ProceduralGenerator';
...
export class MapEditor {
  ...
  private godRayMeshes: Map<string, THREE.Mesh> = new Map();
  private godRaysEnabled = false;
  private shadowsEnabled = false;

  // Animation system
  private animationSystem: AnimationSystem;
  private scriptGraph: ScriptGraph = { nodes: [], connections: [] };
  ...
  public onObjectVisibilityChanged?: (objectId: string, visible: boolean) => void;
  public onLayerChanged?: () => void;
  public onLightPropertiesChanged?: (props: LightProperties | null) => void;
  public onAnimationTimeChanged?: (time: number) => void;
  ...
  constructor(container: HTMLElement) {
    ...
    this.layerSystem.onLayerChanged = () => {
    ...
    this.volumetricLightSystem = new VolumetricLightSystem();
  ...
  private placeObject() {
    ...
    // Initialize light properties for lighting-category objects
    if (this.selectedObjectType.category === 'lighting') {
      this.initLightProperties(id);
    }
  }
  ...
  private clearSelection() {
    ...
    this.onLightPropertiesChanged?.(null);
  }
  ...
  exportMap(): MapData {
    ...
    return {
      name: 'Untitled Map',
      version: 1,
      objects: [...this.placedObjectsData],
      terrain: this.terrainSystem.exportData(),
      waterZones,
      layerAssignments: this.layerSystem.getAssignments(),
      layers: this.layerSystem.exportLayers(),
      animations: this.animationSystem.exportData(),
      scripts: this.scriptGraph,
    };
  }
  ...
  importMap(data: MapData) {
    ...
    if (data.layers) {
      this.layerSystem.importLayers(data.layers);
    }
    if (data.layerAssignments) {
      this.layerSystem.importAssignments(data.layerAssignments);
    }
    if (data.animations) {
      this.animationSystem.importData(data.animations);
    }
    if (data.scripts) {
      this.scriptGraph = data.scripts;
    }
  ...
  applyLayerVisibility(): void {
    ...
    const visibleIds = new Set(this.layerSystem.getVisibleObjects(allIds));
  ...
  setObjectVisibility(objectId: string, visible: boolean): void {
    ...
    if (obj) {
      obj.visible = visible;
      this.onObjectVisibilityChanged?.(objectId, visible);
    }
  }

  assignObjectToLayer(objectId: string, layerId: string): void {
    this.layerSystem.assignObjectToLayer(objectId, layerId);
  }

  getLayerForObject(objectId: string): string {
    return this.layerSystem.getLayerForObject(objectId);
  }

  getMiniMapCanvas(): HTMLCanvasElement {
    return this.miniMap.getCanvas();
  }
  ...
  updateObjectPosition(objectId: string, position: { x: number; y: number; z: number }): void {
    ...
    const data = this.placedObjectsData.find(d => d.id === objectId);
    if (data) {
  ...
  getObjectsByIds(ids: string[]): PlacedObject[] {
    return this.placedObjectsData.filter(d => ids.includes(d.id)).map(d => ({ ...d }));
  }

  placeObjectsFromData(objects: PlacedObject[], offset: { x: number; z: number }): void {
    for (const srcData of objects) {
      const objType = getObjectById(srcData.type);
      if (!objType) continue;
      ...
      const newObj = objType.create();
      pruneShadowCasters(newObj);
      newObj.position.set(srcData.position.x + offset.x, srcData.position.y, srcData.position.z + offset.z);
      newObj.rotation.y = THREE.MathUtils.degToRad(srcData.rotation);
      if (srcData.rotationX) newObj.rotation.x = THREE.MathUtils.degToRad(srcData.rotationX);
      if (srcData.rotationZ) newObj.rotation.z = THREE.MathUtils.degToRad(srcData.rotationZ);
      if (srcData.scale) newObj.scale.setScalar(srcData.scale);
      newObj.userData.editorId = newId;
      newObj.userData.objectType = srcData.type;
      this.scene.add(newObj);
      newObj.updateWorldMatrix(true, true);
      this.placedObjects.set(newId, newObj);
      ...
      const newData: PlacedObject = {
        ...
        position: { x: newObj.position.x, y: newObj.position.y, z: newObj.position.z },
        rotation: srcData.rotation,
        rotationX: srcData.rotationX,
        rotationZ: srcData.rotationZ,
        scale: srcData.scale,
  ...
  getAllObjectIds(): string[] {
    return this.placedObjectsData.map(d => d.id);
  }
  ...
  private animate() {
    ...
    if (this.animationSystem.isPlaying()) {
      this.animationSystem.update(d, (objectId, pos, rot) => {
        const obj = this.placedObjects.get(objectId);
        if (obj) {
          obj.position.set(pos.x, pos.y, pos.z);
          obj.rotation.y = THREE.MathUtils.degToRad(rot);
        }
      });
      this.onAnimationTimeChanged?.(this.animationSystem.getCurrentTime());
    }

    this.renderer.render(this.scene, this.editorCamera.camera);
    this.miniMap.update(this.scene);
  }
  getObjectTypes() { return getAllEditorObjectTypes(); }

  // === LIGHTING METHODS ===
  updateLightProperty(property: string, value: string | number | boolean): void {
    if (!this.selectedObject) return;
    const id = this.selectedObject.userData.editorId as string;
    const data = this.placedObjectsData.find(d => d.id === id);
    if (!data) return;

    const props = this.lightingSystem.getProperties(id);
    if (!props) return;

    this.lightingSystem.updateLightProperty(id, property as keyof LightProperties, value);

    // Store in PlacedObject data
    if (property === 'color') data.lightColor = value as string;
    else if (property === 'intensity') data.lightIntensity = value as number;
    else if (property === 'radius') {
      data.lightRadius = value as number;
      this.lightingSystem.updateHelper(id, value as number, this.scene);
      const helper = this.lightingSystem.getHelper(id);
      if (helper && this.selectedObject) {
        helper.position.copy(this.selectedObject.position);
      }
    }
    else if (property === 'shadows') data.lightShadows = value as boolean;

    const updatedProps = this.lightingSystem.getProperties(id);
    this.onLightPropertiesChanged?.(updatedProps);
  }

  toggleGodRays(enabled: boolean): void {
    this.godRaysEnabled = enabled;
    if (!enabled) {
      // Remove all god ray meshes
      this.godRayMeshes.forEach((mesh) => {
        this.volumetricLightSystem.removeGodRays(mesh, this.scene);
      });
      this.godRayMeshes.clear();
    } else {
      // Create god rays for all lights
      for (const data of this.placedObjectsData) {
        const objType = getObjectById(data.type);
        if (objType && objType.category === 'lighting') {
          const pos = new THREE.Vector3(data.position.x, data.position.y, data.position.z);
          const dir = new THREE.Vector3(0, -1, 0);
          const props = this.lightingSystem.getProperties(data.id);
          const color = props?.color || '#ffffff';
          const intensity = props?.intensity || 1;
          const mesh = this.volumetricLightSystem.createGodRays(pos, dir, color, intensity * 0.3);
          this.scene.add(mesh);
          this.godRayMeshes.set(data.id, mesh);
        }
      }
    }
  }
  ...
  toggleShadows(enabled: boolean): void {
    ...
    this.lightingSystem.toggleShadowPreview(this.renderer, enabled);
  }

  isGodRaysEnabled(): boolean { return this.godRaysEnabled; }
  isShadowsEnabled(): boolean { return this.shadowsEnabled; }

  private initLightProperties(id: string): void {
    const defaults: LightProperties = { color: '#ffffff', intensity: 1, radius: 10, shadows: false };
    this.lightingSystem.setProperties(id, defaults);
    // Create helper
    const helper = this.lightingSystem.createLightHelper(defaults.radius);
    this.lightingSystem.addHelper(id, helper);
  }

  private emitLightProperties(): void {
    if (!this.selectedObject) {
      this.onLightPropertiesChanged?.(null);
      return;
    }
    const id = this.selectedObject.userData.editorId as string;
    const objType = getObjectById(this.selectedObject.userData.objectType);
    if (objType && objType.category === 'lighting') {
      const props = this.lightingSystem.getProperties(id);
      this.onLightPropertiesChanged?.(props);
    } else {
      this.onLightPropertiesChanged?.(null);
    }
  }
  ...
  getAnimationSystem(): AnimationSystem { return this.animationSystem; }
  getScriptGraph(): ScriptGraph { return this.scriptGraph; }
  setScriptGraph(graph: ScriptGraph): void { this.scriptGraph = graph; }

  // === PROCEDURAL GENERATION ===
  generateProcedural(params: GenerationParams): number {
    const generator = new ProceduralGenerator(params.seed);
    const generatedObjects = generator.generateArea(params);

    const placedData: PlacedObject[] = [];
    for (const data of generatedObjects) {
      const t = getObjectById(data.type);
      if (!t) continue;
      const obj = t.create();
      pruneShadowCasters(obj);
    ...
    // Push multi-place as a single undo entry (using multi_delete pattern for undo)
    if (placedData.length > 0) {
      this.pushHistory({ action: 'multi_delete', data: placedData[0], multiData: placedData });
    }

    this.onObjectPlaced?.(this.placedObjectsData.length);
    return placedData.length;
  }

  fillAreaWithObjects(bounds: { x: number; z: number; w: number; d: number }, objectType: string, spacing: number): number {
    const generator = new ProceduralGenerator(Date.now());
    const generatedObjects = generator.fillArea(bounds, objectType, spacing);
  ...
  dispose() { this.stop(); this.lightingSystem.dispose(this.scene); this.terrainSystem.dispose(); this.miniMap.dispose(); this.renderer.domElement.parentElement?.removeChild(this.renderer.domElement); this.renderer.dispose(); this.editorCamera.dispose(); }
}
