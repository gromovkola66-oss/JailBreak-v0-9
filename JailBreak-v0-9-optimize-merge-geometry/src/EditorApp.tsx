...
import { Prefab, PrefabSystem } from './editor/PrefabSystem';
import { LightProperties } from './editor/LightingSystem';
import { HeightmapImporter } from './editor/HeightmapImporter';
import { TerrainTextureSystem } from './editor/TerrainTextureSystem';
import { VegetationBrush, VegetationPreset } from './editor/VegetationBrush';
import { AnimationTrack } from './editor/AnimationSystem';
import { ScriptGraph } from './editor/NodeEditor';
...
export const EditorApp = ({ onBackToGame }: EditorAppProps) => {
  ...
  const [activeLayer, setActiveLayer] = useState<string>('geometry');
  const [isolationLayerId, setIsolationLayerId] = useState<string | null>(null);

  // Prefab system state
  const prefabSystemRef = useRef<PrefabSystem>(new PrefabSystem());
  const [prefabs, setPrefabs] = useState<Prefab[]>(() => prefabSystemRef.current.getPrefabs());

  // MiniMap canvas
  const [miniMapCanvas, setMiniMapCanvas] = useState<HTMLCanvasElement | null>(null);

  // Lighting state
  const [lightProperties, setLightProperties] = useState<LightProperties | null>(null);
  const [godRaysEnabled, setGodRaysEnabled] = useState(false);
  const [shadowsEnabled, setShadowsEnabled] = useState(false);

  // Advanced terrain state
  const heightmapImporterRef = useRef(new HeightmapImporter());
  const terrainTextureSystemRef = useRef(new TerrainTextureSystem(64, 64));

  // Vegetation brush state
  const vegetationBrushRef = useRef(new VegetationBrush());
  const [vegetationPreset, setVegetationPreset] = useState<VegetationPreset>({ type: 'tree', density: 5, scale: [0.8, 1.2], randomRotation: true });

  // Timeline / Animation state
  const [timelineVisible, setTimelineVisible] = useState(false);
  const [nodeEditorVisible, setNodeEditorVisible] = useState(false);
  const [proceduralPanelVisible, setProceduralPanelVisible] = useState(false);
  const [animPlaying, setAnimPlaying] = useState(false);
  const [animCurrentTime, setAnimCurrentTime] = useState(0);
  const [animDuration, setAnimDuration] = useState(0);
  const [animTracks, setAnimTracks] = useState<AnimationTrack[]>([]);
  const [scriptGraph, setScriptGraph] = useState<ScriptGraph>({ nodes: [], connections: [] });
  ...
  useEffect(() => {
    ...
    editor.onLightPropertiesChanged = (props: LightProperties | null) => setLightProperties(props);

    // Animation system integration
    editor.onAnimationTimeChanged = (time: number) => {
      setAnimCurrentTime(time);
      setAnimPlaying(editor.getAnimationSystem().isPlaying());
    };
    setAnimTracks(editor.getAnimationSystem().getTracks());
    setAnimDuration(editor.getAnimationSystem().getDuration());
    setScriptGraph(editor.getScriptGraph());

    // Layer system integration
    setLayers(editor.layerSystem.getLayers());
    editor.onLayerChanged = () => {
      setLayers(editor.layerSystem.getLayers());
      setIsolationLayerId(editor.layerSystem.getIsolationLayerId());
    };

    // MiniMap
    setMiniMapCanvas(editor.getMiniMapCanvas());
  ...
  const handleLayerToggleVisibility = useCallback((layerId: string) => {
    editorRef.current?.layerSystem.toggleVisibility(layerId);
  }, []);
  const handleLayerToggleLock = useCallback((layerId: string) => {
    editorRef.current?.layerSystem.toggleLock(layerId);
  }, []);
  const handleLayerIsolation = useCallback((layerId: string | null) => {
    editorRef.current?.layerSystem.setIsolation(layerId);
  }, []);
  const handleActiveLayerChange = useCallback((layerId: string) => {
    setActiveLayer(layerId);
  }, []);
  const handleAssignLayer = useCallback((layerId: string) => {
    const editor = editorRef.current;
    if (!editor) return;
    if (selectedObject) {
      editor.assignObjectToLayer(selectedObject.id, layerId);
    }
  }, [selectedObject]);

  // Prefab handlers
  const handleSavePrefab = useCallback((name: string) => {
    const editor = editorRef.current;
    if (!editor) return;
    const ids = editor.getAllObjectIds();
    const objs = editor.getObjectsByIds(ids);
    if (objs.length > 0) {
      prefabSystemRef.current.savePrefab(name, objs.slice(0, multiSelectCount));
      setPrefabs(prefabSystemRef.current.getPrefabs());
    }
  }, [multiSelectCount]);
  const handleDeletePrefab = useCallback((id: string) => {
    prefabSystemRef.current.deletePrefab(id);
    setPrefabs(prefabSystemRef.current.getPrefabs());
  }, []);
  const handlePlacePrefab = useCallback((id: string) => {
    const editor = editorRef.current;
    if (!editor) return;
    const prefab = prefabSystemRef.current.getPrefabs().find(p => p.id === id);
    if (prefab) {
      editor.placeObjectsFromData(prefab.objects, { x: 2, z: 2 });
    }
  }, []);

  // Position editing handler
  const handlePositionChange = useCallback((axis: 'x' | 'y' | 'z', value: number) => {
    ...
    editor.updateObjectPosition(selectedObject.id, newPos);
  }, [selectedObject]);

  // Light property handler
  const handleLightPropertyChange = useCallback((property: string, value: string | number | boolean) => {
    editorRef.current?.updateLightProperty(property, value);
  }, []);
  ...
  const handleToggleGodRays = useCallback(() => {
    const editor = editorRef.current;
    if (!editor) return;
    const newState = !godRaysEnabled;
    setGodRaysEnabled(newState);
    editor.toggleGodRays(newState);
  }, [godRaysEnabled]);
  ...
  const handleToggleShadows = useCallback(() => {
    ...
    editor.toggleShadows(newState);
  }, [shadowsEnabled]);

  // Heightmap import handler
  const handleHeightmapImport = useCallback((file: File) => {
    const editor = editorRef.current;
    if (!editor) return;
    heightmapImporterRef.current.importHeightmap(file, editor.getTerrainSystem(), 20).catch(err => {
      console.error('Heightmap import failed:', err);
    });
  }, []);

  // Auto-paint terrain
  const handleTerrainAutoPaint = useCallback(() => {
    const terrainTexture = terrainTextureSystemRef.current;
    const w = terrainTexture.getWidth();
    const d = terrainTexture.getDepth();
    // Create simple height data for auto-paint demo
    const heights = new Float32Array(w * d);
    terrainTexture.autoByHeightSlope(heights, w, d, 20);
  }, []);

  // Vegetation brush
  const handleVegetationPaint = useCallback(() => {
    const editor = editorRef.current;
    if (!editor) return;
    vegetationBrushRef.current.paint(editor.getScene(), 0, 0, 10, vegetationPreset);
  }, [vegetationPreset]);

  const handleVegetationClear = useCallback(() => {
    const editor = editorRef.current;
    if (!editor) return;
    vegetationBrushRef.current.clear(editor.getScene());
  }, []);

  const handleVegetationPresetChange = useCallback((preset: VegetationPreset) => {
    setVegetationPreset(preset);
  }, []);
  ...
  const handleAnimPlay = useCallback(() => {
    const editor = editorRef.current;
    if (!editor) return;
    editor.getAnimationSystem().play();
    setAnimPlaying(true);
  }, []);
  const handleAnimPause = useCallback(() => {
    const editor = editorRef.current;
    if (!editor) return;
    editor.getAnimationSystem().pause();
    setAnimPlaying(false);
  }, []);
  const handleAnimStop = useCallback(() => {
    const editor = editorRef.current;
    if (!editor) return;
    editor.getAnimationSystem().stop();
    setAnimPlaying(false);
    setAnimCurrentTime(0);
  }, []);
  const handleAnimScrub = useCallback((time: number) => {
    const editor = editorRef.current;
    if (!editor) return;
    editor.getAnimationSystem().scrubTo(time);
    setAnimCurrentTime(time);
  }, []);
  const handleAddKeyframe = useCallback(() => {
    const editor = editorRef.current;
    if (!editor || !selectedObject) return;
    const animSys = editor.getAnimationSystem();
    animSys.addKeyframe(selectedObject.id, {
      time: animSys.getCurrentTime(),
      position: { ...selectedObject.position },
      rotation: selectedObject.rotation,
      easing: 'linear',
    });
    setAnimTracks([...animSys.getTracks()]);
    setAnimDuration(animSys.getDuration());
  }, [selectedObject]);
  const handleRemoveKeyframe = useCallback((objectId: string, keyframeIndex: number) => {
    const editor = editorRef.current;
    if (!editor) return;
    const animSys = editor.getAnimationSystem();
    animSys.removeKeyframe(objectId, keyframeIndex);
    setAnimTracks([...animSys.getTracks()]);
    setAnimDuration(animSys.getDuration());
  }, []);
  ...
  const handleToggleNodeEditor = useCallback(() => {
    setNodeEditorVisible(v => !v);
  }, []);
  const handleScriptGraphChanged = useCallback((graph: ScriptGraph) => {
    setScriptGraph(graph);
    editorRef.current?.setScriptGraph(graph);
  }, []);
  ...
  const handleToggleProcedural = useCallback(() => {
    setProceduralPanelVisible(v => !v);
  }, []);
  const handleProceduralGenerate = useCallback((params: GenerationParams) => {
    const editor = editorRef.current;
    if (!editor) return;
    editor.generateProcedural(params);
  }, []);
  const handleProceduralFillArea = useCallback((bounds: { x: number; z: number; w: number; d: number }, objectType: string, spacing: number) => {
    const editor = editorRef.current;
    if (!editor) return;
    editor.fillAreaWithObjects(bounds, objectType, spacing);
  }, []);
    ...
    <div className="w-screen h-screen overflow-hidden bg-black">
      ...
      {mode === 'editing' && (
        ...
        <EditorUI
          ...
          isolationLayerId={isolationLayerId}
          onLayerToggleVisibility={handleLayerToggleVisibility}
          onLayerToggleLock={handleLayerToggleLock}
          onLayerIsolation={handleLayerIsolation}
          ...
          onLightPropertyChange={handleLightPropertyChange}
          onToggleGodRays={handleToggleGodRays}
          onToggleShadows={handleToggleShadows}
          onHeightmapImport={handleHeightmapImport}
          onTerrainAutoPaint={handleTerrainAutoPaint}
          vegetationPreset={vegetationPreset}
          onVegetationPaint={handleVegetationPaint}
          onVegetationClear={handleVegetationClear}
          onVegetationPresetChange={handleVegetationPresetChange}
          timelineVisible={timelineVisible}
          nodeEditorVisible={nodeEditorVisible}
          animPlaying={animPlaying}
          onToggleTimeline={handleToggleTimeline}
          onToggleNodeEditor={handleToggleNodeEditor}
          proceduralPanelVisible={proceduralPanelVisible}
          onToggleProcedural={handleToggleProcedural}
        />
        {/* Timeline Bar */}
        {timelineVisible && (
          <TimelineBar
        ...
        <NodeEditorPanel
          ...
          onGraphChanged={handleScriptGraphChanged}
          onClose={handleCloseNodeEditor}
        />
        {/* Procedural Panel */}
        <ProceduralPanel
          visible={proceduralPanelVisible}
          onGenerate={handleProceduralGenerate}
          onFillArea={handleProceduralFillArea}
        />
        </>
      )}
